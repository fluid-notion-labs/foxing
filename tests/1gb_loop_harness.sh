...
echo "   # Create large file (50MB)"
echo "   dd if=/dev/zero of=$SOURCE_MNT/vm_disk.qcow2 bs=1M count=50"
echo "   wait_for $TARGET_MNT/vm_disk.qcow2"
echo "   "
echo "   # Adversarial: Simulated Guest OS random writes (4KB blocks at random offsets)"
echo "   for i in {1..20}; do"
echo "     OFFSET=\$((RANDOM * 204800 + 4096)) # Random offset within the first 100MB of 50MB disk"
echo "     dd if=/dev/urandom of=$SOURCE_MNT/vm_disk.qcow2 bs=4K count=1 seek=\$((OFFSET / 4096)) conv=notrunc"
echo "   done"
echo "   sync"
