...
// Removed unused import: crate::hydration_worker::HydrationQueue
...
impl Hydrator {
    ...
    fn queue_deletion(&self, rel: &Path) {
        for _ in &self.targets {
            // Need to acquire read lock on queues to access high_q_arc
            let queues = futures::executor::block_on(self.source.queues.read());
            if let Some(high_q_arc) = queues.get(&self.source.dev).and_then(|qs| qs.get(0)) {
                let evt = Event {
    ...
    pub fn process_path(&self, path: &Path, urgent: bool, kind: Option<EventKind>) -> Result<()> {
        ...
        if let Some(EventKind::Modify(_)) = kind {
            // Hot path optimization: simple modifications usually caught by BPF. 
            // Inotify is backup. If strict consistency is needed, remove this return.
            return Ok(());
        }
    ...
    /// Queues an MKNOD event for a special file (block/char/fifo) to the primary event queue (High QoS).
