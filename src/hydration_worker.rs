...
pub struct HydrationJob {
    ...
    pub inode: Option<u64>, // FOXING FIX: Added Inode
}
...
async fn process_hydration_job(
    ...
    // FOXING FIX: Inode-driven path recovery.
    // If we have an inode, trust it more than the path (which might be stale).
            ...
            Ok(new_full_path) => {
                if let Ok(new_rel) = new_full_path.strip_prefix(&source.mount) {
                    if new_rel != rel_path.as_path() {
                        debug!("Hydration: Inode {} moved from {:?} to {:?}. Following.", inode, rel_path, new_rel);
                        rel_path = new_rel.to_path_buf();
                    }
                }
            },
            Err(_) => {
                // If resolving by inode fails, we fall back to the path, 
                // but we DO NOT assume deletion yet.
            }
        }
    }

    let mut source_path = source.mount.join(&rel_path);
    let mut target_path = target_cfg.path.join(&rel_path);

    // Standard Resolution (Path-based)
    if let Ok(metadata) = std::fs::metadata(&source_path) {
        let inode = metadata.ino();
        // Clear tracker if found
        tracker.remove(&inode);
    } else {
        // Path is missing. If we didn't have an inode before, or if the inode lookup failed...
        if !source_path.exists() {
            // Only delete if we are SURE it's gone and not just moved.
            // If we had an inode and lookup failed, it's likely gone.
            // If we didn't have an inode, we trust the path.
