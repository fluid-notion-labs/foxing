...
impl TmpFileGuard {
    fn new(path: PathBuf) -> Self {
        debug!("Atomic Rename Start: {:?} -> {:?}", path, path.with_extension(""));
...
impl SmartCopier {
    pub async fn copy(
        ...
        // FIX 5: Explicitly sync before atomic rename
        if is_full_replace {
            let target_path_clone = target_path.clone();
            let fsync_res = spawn_blocking(move || {
                let f = std::fs::OpenOptions::new().write(true).open(&target_path_clone)?;
                f.sync_all()?;
                // Force filesystem metadata update (optional, but helpful for integrity)
                let _ = f.sync_data(); 
                Ok::<(), io::Error>(())
            }).await;
        ...
        // Original close and rename logic
        unsafe {
            libc::close(dfd);
        }
