...
// FIX #5: Partial Content Hash Calculation
...
// ... (Existing probe and util functions preserved) ...
...
pub fn probe_xattr_support(target_root: &Path) -> bool {
    ...
    if let Ok(f) = File::create(&probe_file) {
...
pub fn probe_rwf_uncached(target_root: &Path) -> bool {
    ...
    let sfd = match file_res {
        Ok(f) => f.as_raw_fd(),
        Err(e) => {
            error!("Probe RWF_UNCACHED: Failed to open temp file: {}", e);
            let _ = std::fs::remove_file(probe_file);
            return false;
        }
    };
    ...
    if ret != 4096 {
        error!("Probe RWF_UNCACHED: Failed initial write.");
        let _ = std::fs::remove_file(probe_file);
        return false;
    }
    ...
    let result = match ring.submit_and_wait(1) {
        ...
        Err(e) => {
            error!("Probe RWF_UNCACHED: Failed submit_and_wait: {}", e);
            false
        }
    };
    let _ = ring.submitter().unregister_buffers();
...
pub fn create_version_snapshot(path: &Path, epoch_seq: u64, root_path: &Path, inode: u64) -> Result<()> {
    ...
    // FIX #5: Calculate partial hash for snapshot metadata
