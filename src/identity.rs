...
pub fn resolve_and_update_path(map: &InodeMap, source_mount_root: &std::path::Path, inode: u64) -> Result<PathBuf, io::Error> {
    ...
    if let Some(path) = found_path {
        if fs::metadata(source_mount_root.join(&path)).is_ok() {
            // Note: We no longer update the map here. The caller (worker) handles that to
            // control the poisoning vector. We just return the path found.
            return Ok(path);
        }
    }
