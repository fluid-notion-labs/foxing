...
impl BbrHistory {
    fn new(window_secs: u64) -> Self {
        Self {
            ...
            max_gap: Duration::from_millis(1), // Reduced min gap to better track rapid events
        }
    }
    ...
    fn recommended_idle_timeout(&self) -> Duration {
        ...
        derived.max(Duration::from_millis(500)).min(Duration::from_secs(300)) // Reduced idle floor
    }
}
pub struct VdoTuner {
    enabled: bool,
...
impl BbrTuner {
    pub fn new(cfg: &TargetConfig) -> Self {
        ...
        Self {
            ...
            hydration_debounce: Duration::from_millis(50), // Defaulting to a faster debounce for better repair response
    ...
    pub fn tune(&mut self, elapsed_secs: f64, bytes_processed: u64, is_stressed: bool, pending_len: usize, max_pending: usize, board: &TunerBoard, path_label: &str, chunk_size: u64) -> usize {
        ...
        let (batch_override, coalesce_override, target_inflight_bytes) = match effective_state {
            ...
            TunerState::Muted => {
                 // Aggressively drain queue to clear overload condition faster
                 (self.batch_size_min, self.min_coalesce_floor, 0)
            },
            _ => {
                ...
                let pacing_gain = match self.state {
                    ...
                    TunerState::Muted => 0.75, // Should be caught by the above match arm, but here for safety
                    _ => 1.0,
                };
                let target_inflight_bytes = (bdp_bytes * pacing_gain) as u64;
                // Base calculation for ideal operation size
