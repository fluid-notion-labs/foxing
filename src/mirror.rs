...
fn calculate_adaptive_registry_limit(tuner_board: &TunerBoard) -> usize {
    for r in tuner_board.iter() {
        if matches!(*r.value(), TunerState::CriticalDrain) { 
            return 10; 
        }
    }
    1000
        ...
        for (_primary_dev, src) in self.sources.iter_mut() {
            if let Some(source_cfg) = config_reader.sources.iter().find(|s| s.path == src.path) {
                ...
                for tgt_cfg in &source_cfg.targets {
                    // FIX: Architecture C (Lock Contention)
                    // SerializationEngine is now created PER TARGET but could be shared if architecture demands.
                    // For now, retaining per-target to allow concurrent writes to DIFFERENT targets.
                    // If shared contention is on SOURCE read, OS handles that.
